...
class TashkeelTokenizer:
    ...
    def clean_text(self, text):
        text = re.sub("[%s]" % "\u0640", "", text)  # strip tatweel
    ...
    def check_match(self, text_with_tashkeel, letter_n_tashkeel_pairs):
        ...
        return (
            syn_text == text_with_tashkeel
            or syn_text == self.unify_shaddah_position(text_with_tashkeel)
        )
    ...
    def unify_shaddah_position(self, text_with_tashkeel):
        ...
        for i in range(len(self.shaddah_first)):
            text_with_tashkeel = text_with_tashkeel.replace(
                self.shaddah_last[i], self.shaddah_first[i]
            )
    ...
    def split_tashkeel_from_text(self, text_with_tashkeel, test_match=True):
        ...
        for i in range(len(self.tahkeel_chars)):
            text_with_tashkeel = text_with_tashkeel.replace(
                self.tahkeel_chars[i] * 2, self.tahkeel_chars[i]
            )
        ...
        for i in range(len(text_with_tashkeel)):  # go over the whole text
            ...
            if (
                i < (len(text_with_tashkeel) - 1)
                and not text_with_tashkeel[i] in self.tashkeel_list
                and text_with_tashkeel[i + 1] in self.tashkeel_list
            ):
                ...
                if text_with_tashkeel[i + 1] == "~":
                    ...
                    if (
                        i + 2 < len(text_with_tashkeel)
                        and f"~{text_with_tashkeel[i+2]}" in self.inverse_tags
                    ):
                        letter_n_tashkeel_pairs.append(
                            (
                                text_with_tashkeel[i],
                                self.inverse_tags[f"~{text_with_tashkeel[i+2]}"],
                            )
                        )
                    else:
                    ...
                    letter_n_tashkeel_pairs.append(
                        (text_with_tashkeel[i], text_with_tashkeel[i + 1])
                    )
            ...
            elif text_with_tashkeel[i] not in self.tashkeel_list:
                letter_n_tashkeel_pairs.append(
                    (text_with_tashkeel[i], self.no_tashkeel_tag)
                )
    ...
    def encode(self, text_with_tashkeel, test_match=True):
        letter_n_tashkeel_pairs = self.split_tashkeel_from_text(
            text_with_tashkeel, test_match
        )
    ...
    def decode(self, input_ids, target_ids):
        ...
        for j in range(len(input_ids)):
            ...
            letters = list(
                filter(
                    lambda x: x != "<BOS>" and x != "<EOS>" and x != "<PAD>", letters
                )
            )
            tashkeel = self.filter_tashkeel(tashkeel)
            tashkeel = list(
                filter(
                    lambda x: x != "<BOS>" and x != "<EOS>" and x != "<PAD>", tashkeel
                )
            )
    ...
    def get_tashkeel_with_case_ending(self, text, case_ending=True):
        ...
        text_spaces_indecies = [
            i for i, el in enumerate(text_split) if el == (" ", "<NT>")
        ]
        new_text_split = []
    ...
    def compute_der(self, ref, hyp, case_ending=True):
        _, ref_tashkeel = self.get_tashkeel_with_case_ending(
            ref, case_ending=case_ending
        )
        _, hyp_tashkeel = self.get_tashkeel_with_case_ending(
            hyp, case_ending=case_ending
        )
    ...
    def compute_wer(self, ref, hyp, case_ending=True):
        ref_letters, ref_tashkeel = self.get_tashkeel_with_case_ending(
            ref, case_ending=case_ending
        )
        hyp_letters, hyp_tashkeel = self.get_tashkeel_with_case_ending(
            hyp, case_ending=case_ending
        )
        ref_text_combined = self.combine_tashkeel_with_text(
            zip(ref_letters, ref_tashkeel)
        )
        hyp_text_combined = self.combine_tashkeel_with_text(
            zip(hyp_letters, hyp_tashkeel)
        )
