...
class TashkeelModel(pl.LightningModule):
    def __init__(self, tokenizer, max_seq_len, d_model=512, n_layers=3, n_heads=16, drop_prob=0.1):
    ...
    @torch.no_grad()
    def do_tashkeel_batch(self, texts, batch_size=16):
        self.eval()
        device = next(self.parameters()).device
        text_with_tashkeel = []
        num_batches = math.ceil(len(texts) / batch_size)
        for texts_mini in tqdm(get_batches(texts, batch_size), total=num_batches):
            input_ids_list = []
    ...
    def do_tashkeel(self, text):
        self.eval()
        device = next(self.parameters()).device
#        print('Tashkeel is done on device:', device)
        input_ids, _ = self.tokenizer.encode(text)
        input_ids = input_ids.to(device) #.unsqueeze(0)
        target_ids = torch.LongTensor([[self.tokenizer.tashkeel_map['<BOS>']]]).to(device)

        src = input_ids.unsqueeze(0)
...
#            print('trg.shape', trg.shape)
            ...
            preds = self.transformer.decoder(trg, enc_src, trg_mask, src_trg_mask)
            #preds = self(input_ids.unsqueeze(0), target_ids)
            target_ids = torch.cat([target_ids, preds[:, -1].argmax(1).unsqueeze(0)], axis=1)
        text_with_tashkeel = self.tokenizer.decode(src, target_ids)
        return text_with_tashkeel
